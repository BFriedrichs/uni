...
getHigher:
  MOV(AL, AT_DISP16( mn_key ))          ; Move scan code to AL
  SHR(AL,IM8(4))                        ; Right shift to get rid of lower 4 bits
  CMP(AL,IM8(9))                        ; Compare higher 4 bits to 9 to be tell if we display a number or a letter
  JG(greater)                           ; Conditional jump if it was greater
  ADD(AL,IM8('0'))                      ; If not add the ASCII value of '0' (48) to our number
  JMP(displayGreater)                   ; Jump to display to skip "greater"

greater:
  ADD(AL,IM8('A'-10))                   ; Code was greater than 9 so we have to add 'A' - 10
                                        ; -10 is necessary because if we want to display an 'A' the decimal value
                                        ; for an A is 10.

displayGreater:
  SEG( ES ) MOV( AT_DISP16( 80 * 2 ), AL )  ; Use segment ES and write character at position 80 * 2
                                            ; which corresponds to the first character of the second row,
                                            ; since our output is 80x25

getLower:
  MOV(AL, AT_DISP16( mn_key ))          ; Reload scancode
  AND( AL, IM8( 0b00001111 ) )          ; Instead of shifting out bits we use AND to only keep the lower 4 bits
  CMP(AL,IM8(9))                        ; Compare, Jump, and Add for the same reasons of higher bits
  JG(lowerGreater)
  ADD(AL,IM8('0'))
  JMP(displayLower)
...
displayLower:
  SEG( ES ) MOV( AT_DISP16( 80 * 2 + 2), AL ) ; This time display at 80 * 2 + 2 to write to the second position in the second row
  JMP(MN_WAIT_FOR_KEY)                        ; Jump back into the WAIT_FOR_KEY loop
...
; Control word (address: 43h):
;                                ; Bit:   Ch. 0   Ch. 1   Ch. 2
;   Modes:                       ;  7 |     0       0       1
;    0: Countdown w. inhibit*.   ;    +-->
;    1: Count down w. restart*.  ;  6 |     0       1       0
;    2: One pulse out of N.      ;     Latch cur.  RW MSB  RW LSB   RW LSB,MSB
;    3: Square wave.             ;  5 |     0       0        1         1
;    4: Countdown w. inhibit**.  ;    +-->
;    5: Count down w. restart**. ;  4 |     0       1        0         1
;                                ;
;   *: level output.             ;  3 |
;   **: pulse output.            ;  2 +--> mode: 0 to 5
;                                ;  1 |
;                                ;     Mode:
;                                ;  0    0=binary; 1=BCD

; The clock is 1.19318MHz for all three channels.
